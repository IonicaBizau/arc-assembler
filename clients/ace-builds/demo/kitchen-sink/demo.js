...
define("kitchen-sink/inline_editor", ["require", "exports", "module", "ace/line_widgets", "ace/editor", "ace/virtual_renderer", "ace/lib/dom", "ace/commands/default_commands"], function (require, exports, module) {
    ...
    require("ace/commands/default_commands").commands.push({
        ...
        exec: function exec(editor) {
            ...
            var kb = {
                handleKeyboard: function handleKeyboard(_, hashId, keyString) {
...
define("kitchen-sink/dev_util", ["require", "exports", "module"], function (require, exports, module) {
    ...
    def(window, "ace", function () {
        warn();return window.env.editor;
    });
    def(window, "editor", function () {
        warn();return window.env.editor;
    });
    def(window, "session", function () {
        warn();return window.env.editor.session;
    });
    def(window, "split", function () {
        warn();return window.env.split;
    });
});
...
define("kitchen-sink/doclist", ["require", "exports", "module", "ace/edit_session", "ace/undomanager", "ace/lib/net", "ace/ext/modelist"], function (require, exports, module) {
    ...
    modelist.modes.forEach(function (m) {
        ...
        } else if (_typeof(docs[path]) == "object" && !docs[path].name) {
            docs[path].name = m.caption;
        }
    });
    ...
    if (window.require && window.require.s) try {
        for (var path in window.require.s.contexts._.defined) {
            if (path.indexOf("!") != -1) path = path.split("!").pop();else path = path + ".js";
            ownSource[path] = "";
        }
    } catch (e) {}
    ...
    function prepareDocList(docs) {
        ...
        for (var path in docs) {
            var doc = docs[path];
            if ((typeof doc === "undefined" ? "undefined" : _typeof(doc)) != "object") doc = { name: doc || path };
    ...
    function loadDoc(name, callback) {
        ...
        if (parts[0] == "docs") path = "demo/kitchen-sink/" + path;else if (parts[0] == "ace") path = "lib/" + path;
...
", "splitEditor");
    ...
    var SplitRoot = function SplitRoot(el, theme, position, getSize) {
...
define("kitchen-sink/util", ["require", "exports", "module", "ace/lib/dom", "ace/lib/event", "ace/edit_session", "ace/undomanager", "ace/virtual_renderer", "ace/editor", "ace/multi_select"], function (require, exports, module) {
    ...
    exports.createSplitEditor = function (el) {
        ...
        split.onMouseDown = function (e) {
            ...
            var onResizeInterval = function onResizeInterval() {
    ...
    exports.bindCheckbox = function (id, callback, noInit) {
        if (typeof id == "string") var el = document.getElementById(id);else {
            var el = id;
            id = el.id;
        }
    ...
    module.exports = {
        ...
        "T": new Motion({
            ...
            getPos: function getPos(editor, range, count, param, isSel, isRepeat) {
                ...
                if (isRepeat && column === 0 && !(count > 1)) column = util.getLeftNthChar(editor, cursor, param, 2);
        ...
        "~": {
            fn: function fn(editor, range, count) {
                repeat(function () {
                    ...
                    if (toggled != text) editor.session.replace(range, toggled);else if (text.toLowerCase() != text) editor.session.replace(range, text.toLowerCase());else editor.navigateRight();
                }, count || 1);
            }
        },
        "*": {
        ...
        exec: function exec(editor, action, param) {
            ...
            if (o && !editor.selection.isEmpty()) {
                ...
                if (!o) {
                    ...
                    repeat(function () {
                        run(motionObj.sel);
                        operators[o.ch].fn(editor, editor.getSelectionRange(), o.count, motionObj.param ? motionObj : param);
                    }, o.count || 1);
                }
                this.reset();
            } else if (a) {
            ...
            if (key == "ctrl-c") {
                if (!useragent.isMac && editor.getCopyText()) {
                    editor.once("copy", function () {
                        if (vimState == "start") coreCommands.stop.exec(editor);else editor.selection.clearSelection();
                    });
...
define("ace/autocomplete", ["require", "exports", "module", "ace/keyboard/hash_handler", "ace/autocomplete/popup", "ace/autocomplete/util", "ace/lib/event", "ace/lib/lang", "ace/lib/dom", "ace/snippets"], function (require, exports, module) {
    ...
    (function () {
        ...
        this.blurListener = function (e) {
            ...
            if (el != text && el.parentNode != this.popup.container && el != this.tooltipNode && e.relatedTarget != this.tooltipNode && e.relatedTarget != text) {
                this.detach();
            }
        };
...
border:1px solid #baf; z-index:100";
    ...
    env.editor.commands.addCommands([{
        ...
        exec: function exec(editor, needle) {
            if ((typeof needle === "undefined" ? "undefined" : _typeof(needle)) == "object") {
    ...
    cmdLine.commands.bindKeys({
        "Shift-Return|Ctrl-Return|Alt-Return": function ShiftReturnCtrlReturnAltReturn(cmdLine) {
            cmdLine.insert("\n");
        },
        "Esc|Shift-Esc": function EscShiftEsc(cmdLine) {
            cmdLine.editor.focus();
        },
        "Return": function Return(cmdLine) {
    ...
    doclist.cycleOpen = function (editor, dir) {
        var h = this.history;
        h.index += dir;
        if (h.index >= h.length) h.index = 0;else if (h.index <= 0) h.index = h.length - 1;
        var s = h[h.index];
        docEl.value = s;
        docEl.onchange();
    };
    ...
    event.addListener(themeEl, "mouseover", function (e) {
        ...
        if (!themeEl.$timer) themeEl.$timer = setTimeout(themeEl.updateTheme);
    });

    event.addListener(themeEl, "mouseout", function (e) {
        themeEl.desiredValue = null;
        if (!themeEl.$timer) themeEl.$timer = setTimeout(themeEl.updateTheme, 20);
    });
