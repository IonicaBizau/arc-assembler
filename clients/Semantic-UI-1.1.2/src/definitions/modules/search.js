    ...
    $(this).each(function () {
      ...
      module = {
        ...
        initialize: function initialize() {
          ...
          $results.on('mousedown' + eventNamespace, module.event.mousedown).on('mouseup' + eventNamespace, module.event.mouseup).on('click' + eventNamespace, selector.result, module.results.select);
          module.instantiate();
        },
        ...
        search: {
          ...
          cache: {
            read: function read(name) {
              ...
              return settings.cache && (typeof cache === 'undefined' ? 'undefined' : _typeof(cache)) == 'object' && cache[name] !== undefined ? cache[name] : false;
            },
        ...
        results: {
          generate: function generate(response) {
            module.debug('Generating html from response', response);
            var template = settings.templates[settings.type],
                html = '';
            if ($.isPlainObject(response.results) && !$.isEmptyObject(response.results) || $.isArray(response.results) && response.results.length > 0) {
              if (settings.maxResults > 0) {
                response.results = $.isArray(response.results) ? response.results.slice(0, settings.maxResults) : response.results;
              }
          ...
          select: function select(event) {
            ...
            if (settings.onSelect == 'default' || $.proxy(settings.onSelect, this)(event) == 'default') {
              var $link = $result.find('a[href]').eq(0),
                  ...
                  name = $title.size() > 0 ? $title.text() : false;
              module.results.hide();
              if (name) {
                $prompt.val(name);
              }
